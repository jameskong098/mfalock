...
1.  **Raspberry Pi 1 (Web Server & Sensor Hub):**
    ...
    *   Connects to the 3.5" LCD Touchscreen, which runs `display/test_lcd.py`. This script provides a local user interface for:
        *   Selecting authentication methods: Keypad, Facial Recognition, Voice Recognition, and viewing status for Touch/Rotary.
        *   Entering PINs for Keypad authentication.
        *   Initiating Facial Recognition (running `camera/face_recognition_code.py` as a subprocess).
        *   Initiating Voice Recognition (running `audio/utils/audio_utils.py` as a subprocess to get a challenge phrase and verify).
    *   `display/test_lcd.py` communicates with `web_UI/web_server.py` via Socket.IO to:
        *   Send `lcd_mode_update` events when the user navigates the LCD menu (e.g., selecting "Keypad" or "Facial Recognition"). The web server then broadcasts this as a `sensor_mode_change` to update the "Current Mode" on the web dashboard.
        *   Send `keypad_update` events with the digits being entered on the LCD keypad, which the web server broadcasts as `display_keypad_digits` for live viewing on the dashboard.
        *   Send `voice_phrase_update` events containing the challenge phrase generated by the audio script. The web server broadcasts this as `display_voice_phrase` to the dashboard.
        *   Send `auth_event` messages for authentication attempts (success/failure) initiated via the LCD (Keypad, Facial, Voice). These are logged by the web server and broadcast to the dashboard's live event feed.
    *   Communicates via USB/Serial with a Raspberry Pi Pico running `pico_sensors/all_sensors.py`. This Pico handles inputs from the Touch Sensor and Rotary Angle Sensor. The `web_server.py` monitors the Pico's output for touch/rotary events and mode changes, relaying them to the web UI.
    *   Processes authentication attempts based on inputs from various sensors (Pico, Camera, Microphone, LCD).
    *   Sends authentication status messages (e.g., "TOUCH - SUCCESS", "VOICE - SUCCESS", "FACIAL RECOGNITION - SUCCESS", "KEYPAD - SUCCESS") over the network to Raspberry Pi 2 if the authentication is successful.
...
*   **`display/`**: Contains code related to the touchscreen LCD display (`test_lcd.py`). This script manages the user interface on the LCD, handles keypad input for PIN authentication, initiates facial and voice recognition processes, and displays their status.
...
*   **Root Directory:**
    ...
    *   `settings.json`: Stores system settings configured via the Web UI (e.g., touch pattern, rotary sequence, keypad PIN).
    *   `README.md`: This file.
...
A conventional PIN-based system using the touchscreen display. The PIN is entered via an on-screen keypad managed by `display/test_lcd.py` and the PIN is stored in `web_UI/settings.json`.
